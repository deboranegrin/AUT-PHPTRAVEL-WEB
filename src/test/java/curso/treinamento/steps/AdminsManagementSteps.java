...
//	Instanciar pages
	...
	public void administrador_cadastrado_com_sucesso() {
		...
		Assert.assertTrue("Usuário não cadastrado com sucesso.",
				adminsManagementPage.validar_email_na_tabela(Helper.dataTable.get(0).get("Email")));
	...
	public void eu_incluo_um_administrador_com_os_valores(String firstName, String lastName, String email,
			String password, String mobileNumber, String country, String address1, String address2, String status,
			String add, String edit, String remove) {
	...
	public void edito_um_administrador(DataTable dataTable_) {
		...
		updateAdminPage.limpar_campos("address1");
		updateAdminPage.preencher_address1(Helper.dataTable.get(0).get("Address 1"));
		updateAdminPage.limpar_campos("address2");
	...
	public void administrador_editado_com_sucesso() {
		...
		Assert.assertTrue("Usuário não editado com sucesso.",
				updateAdminPage.validar_email_na_tabela(Helper.dataTable.get(0).get("Email")));

	}

	@Quando("excluo um administrador")
	public void excluo_um_administrador(DataTable dataTable_) {

		Helper.create_datatable(dataTable_);
				
		adminsManagementPage.clicar_btn_delete(Helper.dataTable.get(0).get("Email"));
		adminsManagementPage.remover_admin();

		Assert.assertTrue("Usuário não encontrado.",
				updateAdminPage.validar_email_na_tabela(Helper.dataTable.get(0).get("Email")));
	...
	public void administrador_excluido_com_sucesso() {
		...
		Assert.assertEquals("Changes Saved!", adminsManagementPage.get_mensagem());

		Assert.assertTrue("Usuário não excluído com sucesso.",
				updateAdminPage.validar_email_na_tabela(Helper.dataTable.get(0).get("Email")));
